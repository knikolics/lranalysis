...
void FindPe(const char *filename)
{
  ...
  FunPe = new TF1("FunPe",FunctionPe,-10,10,6);
...
//  FunPe->SetParameters(2.70815e+05,3.33626e-02,6.59402e+03,5.81327e-02,5.20817e-05,3.24655e-02);
  ...
  fSkellam = new TF1("Kellam",Skellam2Pe,-10,10,6);
  fSkellam->SetParameters(FunPe->GetParameters());
  fPoisson = new TF1("Poisson",PoissonPe,-10,10,6);
...
//   }
  ...
  // total distribution
  TH1F *totf = new TH1F("totf","totf",10,0,10);
  Data->Draw("eNpe>>totf");
  totf->Draw();
  //  printf("%d\n",totf->GetNbinsX());

  // taking the noise
  TH1F *noise = new TH1F("noise","noise",10,0,10);
  for (int i=0;i<10;i++) {
