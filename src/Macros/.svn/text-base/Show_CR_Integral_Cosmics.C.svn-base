...
#define MAXINTsingle  100.
#define MAXINTsum     2500.

Float_t CR_min = 0.2;
Float_t CR_max = 0.5;
Float_t Integral_min = 800.; // nVs
Float_t Integral_max = 1500.; // nVs

void Show_CR_Integral_Cosmics(const char *filename, int pe = 0, float conv = 0.2)
{
  if (!pe) conv = 1.;
  ...
  for (Int_t i=0;i<14;i++)
    {
      ...
      frame[i] = new TH2F(framename, framename, 100, 0/conv, MAXINTsingle/conv, 100, 0, 2);
...
//      sprintf(selection[5],"&&VPMT.eIntegral>%f&&VPMT.eIntegral<%f",Integral_min,Integral_max);
  ...
  c->cd(15)->SetPad(1-SizePad*1.3,0,1,SizePad*1.3);
  //  c->cd(NPMTs+1)->SetFillColor(30);

  sprintf(framename,"VPMT",i);

  frame[NPMTs] = new TH2F(framename, framename, 100, 0/conv, MAXINTsum/conv, 100, 0, 1);
  ...
  c->cd(16)->SetPad(0,1-SizePad/5,SizePad/5,1);
}
