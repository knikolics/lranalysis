...
Float_t CR_min = 0.1;
Float_t CR_max = 0.3;
Float_t Integral_min = 70.;
Float_t Integral_max = 170.;
...
void Show_CR_Integral(const char *filename, int pe = 0, float conv = 0.2)
{
  ...
  Int_t sqrtn = (int)(sqrt(MAXPMTS)+0.5);
  ...
  posX[8]  =-2*Sqrt3*Dist; posY[8]  =-1*Dist;
  posX[9]  =-2*Sqrt3*Dist; posY[9]  = 1*Dist;

  posX[5]  =-Sqrt3*Dist;   posY[5]  =-2*Dist; //
  posX[7]  =-Sqrt3*Dist;   posY[7]  = 0*Dist; //
  posX[6]  =-Sqrt3*Dist;   posY[6]  = 2*Dist; //

  posX[3]  = 0;            posY[3]  =-3*Dist; //
  posX[4]  = 0;            posY[4]  =-1*Dist; //
  posX[2]  = 0;            posY[2]  = 1*Dist; //
  posX[10] = 0;            posY[10] = 3*Dist;

  posX[0]  = Sqrt3*Dist;   posY[0]  =-2*Dist; //
  posX[1]  = Sqrt3*Dist;   posY[1]  = 0*Dist; //
  ...
  for (Int_t i=0;i<NPMTs;i++)
    {
      ...
      frame[i] = new TH2F(framename, framename, 100, 0/conv, 300/conv, 100, 0, 1);
      ...
      sprintf(varexp[0],"PMT[%d].eCR:PMT[%d].eIntegral/%f",i,i,conv);
...
//       sprintf(selection[3],"&&PMT[%d].eIntegral>0&&PMT[%d].eIntegral<30",i,i);
  ...
  c->cd(MAXPMTS+1)->SetPad(1-SizePad,0,1,SizePad);
  //  c->cd(MAXPMTS+1)->SetFillColor(30);

  sprintf(framename,"VPMT",i);

  frame[MAXPMTS] = new TH2F(framename, framename, 100, 0/conv, 200/conv, 100, 0, 1);
  if (!pe) frame[MAXPMTS]->GetXaxis()->SetTitle("Integral [nVs]");
  else frame[MAXPMTS]->GetXaxis()->SetTitle("Integral [pe]");
  frame[MAXPMTS]->GetYaxis()->SetTitle("L_{50}/L_{tot}");

  sprintf(varexp[0],"VPMT.eCR:VPMT.eIntegral/%f",conv);
  ...
  sprintf(selection[2],"&&VPMT.eCR>0&&VPMT.eCR<1");
  ...
  c->cd(MAXPMTS+2)->SetPad(0,1-SizePad/10,SizePad/10,1);
}
