...
void ShowSerie(const char *filename)
{
  gStyle->SetOptStat(0);
  ...
  //Tau2 dependence on time  
  TCanvas *ct= new TCanvas("ct","ct",800,800);
  TGraphErrors *gt[MAXPMTS];
  ...
  for (Int_t ch=0;ch<NPMTs[0]+1;ch++){
    ct->cd(1);
    gt[ch]=new TGraphErrors(nruns,Timestamp,&tau2[ch][0],STimestamp,&Stau2[ch][0]);
    gt[ch]->Draw("PL");
