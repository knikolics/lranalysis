...
void ArPMT :: Print(FILE *fp)
{
  ...
  fprintf(fp,"         Name:         %s\n",         eName.Data());
/*  //while(eNPeaks < maxpeaks) { // maxpeak == len
  while(eNPeaks < len) {
  fprintf(fp,"         Type:         %s\n",         eType.Data());
...
void ArPMT :: FindPeaks(Int_t start, Int_t stop) //luillo
{
  ...
  ePeakIntegral.clear();
  ePeakAmplitude.clear();
  ePeakTimes.clear();
  eSumPeakIntegralPMT=0;
  ...
  float NSIGMA = ArConf->PeakFindingSigmaThreshold;

  int maxNrej = 10;
  Int_t minwidth = (int)(ArConf->PeakFindingDisentangleTime); //minimum peak width to be considered a peak = DISENTANGLE_TIME
...
//  bool in_a_peak = false;
  ...
  //    Peaks.AddAt(ploc, eNPeaks);
	...
	cout<<"Peak found "<<eNPeaks<<" "<<pamp<<" "<<ploc<<" "<<pwid<<endl;
      ...
      eSumPeakIntegralPMT += brSum;
      ePeakIntegral.push_back(brSum);
      ePeakAmplitude.push_back(pamp);
      ePeakTimes.push_back(ploc);
    }
    else {
      nrej++;
      if (nrej >= maxNrej) {
